...
print(summary(model, [(3,32,32), (3,32,32)]))
...
for epoch in range(opt.epochs):
    ...
    with torch.no_grad():
        ...
        if ((mean_val_metrics['cd_precisions'] > best_metrics['cd_precisions'])
            ...
            # Save model and log
            if not os.path.exists('./tmp6'):
                os.mkdir('./tmp6')
            with open('./tmp6/metadata_epoch_' + str(epoch) + '.json', 'w') as fout:
                json.dump(metadata, fout)

            torch.save(model, './tmp6/checkpoint_epoch_'+str(epoch)+'.pt')
