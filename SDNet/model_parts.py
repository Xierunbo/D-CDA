...
class _context_block_(nn.Module) :
    def __init__(self, in_channels, kernel_size, stride, dilation, bias) :
        ...
        self._bottleneck_ = _conv_(in_channels , in_channels, 1, stride, dilation, bias)
...
class _ResBlock_CBAM_(nn.Module) :
    ...
    def forward(self, x) :
        out1 = self._conv_in_(x)
        cam = out1 * self._cam_(out1)
        sam = out1 * self._sam_(out1)
        out = cam +sam
